...
//            System.out.println("Rajan_google_login_detail"+acct.getId().toString()+acct.getId()+ acct.getDisplayName().toString()+"google"+photo);
            ...
            Toast.makeText(MyApp, acct.getDisplayName(), Toast.LENGTH_SHORT).show();
    ...
    private void signUp(String toString, String strEmail, String strName, String strPromocode, String google, String photo) {
        try {
            if (JsonUtils.isNetworkAvailable(ActivitySignIn.this)) {
                strPassword = "12345";
                Log.d("TAG", "signUp: " + Constant.REGISTER_URL_GOOGLE
                        + strName + "&email=" + strEmail + "&password=" + strPassword + "&phone=" + toString + "&promocode=" + strPromocode + "&pkg=" + getPackageName());
                new MyTaskRegister()
                        .execute
                                (Constant.REGISTER_URL_GOOGLE
                                        + strName + "&email=" + strEmail + "&password=" + strPassword + "&phone=" + toString + "&promocode=" + strPromocode + "&pkg=" + getPackageName());
            }
        } catch (Exception e) {
            e.printStackTrace();
